...
TEnteromorphaSp::TEnteromorphaSp(TEcoDynClass* APEcoDynClass, char* className)
								 : TRuppiaCirrhosa(APEcoDynClass, className)
{
    NumberOfBoxesWithEnteromorphaSp = 0;
    for (int i = 0; i < NumberOfBoxesWithBenthicSpecies; i++)
    {
        if (strcmp(MyBenthicSpeciesRecord[i].SpeciesName, "Enteromorpha sp") == 0)
            NumberOfBoxesWithEnteromorphaSp++;

    }
    EnteromorphaSpRecord = new BenthicSpeciesRecord[NumberOfBoxesWithEnteromorphaSp];
    int AIndex = 0;
    for (int i = 0; i < NumberOfBoxesWithBenthicSpecies; i++)
    {
        if (strcmp(MyBenthicSpeciesRecord[i].SpeciesName, "Enteromorpha sp") == 0)
        {
            strcpy(EnteromorphaSpRecord[AIndex].SpeciesName, MyBenthicSpeciesRecord[i].SpeciesName);
            EnteromorphaSpRecord[AIndex].LineCoordinate = MyBenthicSpeciesRecord[i].LineCoordinate;
            EnteromorphaSpRecord[AIndex].ColumnCoordinate = MyBenthicSpeciesRecord[i].ColumnCoordinate;
            AIndex++;
        }
    }
    ...
    TReadWrite* PReadWrite = (TReadWrite*)OpenVariablesFile("EnteromorphaSp");
    if (PReadWrite == NULL)
    {
#ifdef __BORLANDC__
		MessageBox(0, "EnteromorphaSp variables file missing.", "EcoDynamo alert", MB_OK);
#else  // __BORLANDC__
		cerr << "TEnteromorphaSp::TEnteromorphaSp - variables file missing" << endl;
#endif  // __BORLANDC__
	}
    else {
        if (PReadWrite->FindString("EnteromorphaSp", X, Y))
        {
    ...
    /*            for (int i = 0; i < NumberOfVariables; i++)
            ...
            for (int i = 0; i < NumberOfVariables; i++)
            {
                ...
                if (strcmp(MyVariable, "EnteromorphaSpBiomass") == 0)
                {
                    if (!PReadWrite->FindString("EnteromorphaSpBiomass values", XV, YV))
                    {
                        XV = X + VARIABLENAMECOLUMN + i;
                        YV = Y;
                    }
                    for (int j = 0; j < NumberOfBoxesWithEnteromorphaSp; j++)
                    {
                        Index = EnteromorphaSpRecord[j].LineCoordinate * NumberOfColumns +
                                EnteromorphaSpRecord[j].ColumnCoordinate;
                        PReadWrite->ReadNumber(XV, j + YV + 1, PhytoBiomass[Index]);
                        //if (j == NumberOfBoxesWithEnteromorphaSp - 1) Debugger(PhytoBiomass[Index]);
                    }
                }
                else
                if (strcmp(MyVariable, "EnteromorphaSpCBiomass") == 0)
                {
                    if (!PReadWrite->FindString("EnteromorphaSpCBiomass values", XV, YV))
                    {
                        XV = X + VARIABLENAMECOLUMN + i;
                        YV = Y;
                    }
                    for (int j = 0; j < NumberOfBoxesWithEnteromorphaSp; j++)
                    {
                        ...
                        //if (j == NumberOfBoxesWithEnteromorphaSp - 1) Debugger(CPhyto[Index]);
                    }
                }
                else
                if (strcmp(MyVariable, "EnteromorphaSpNBiomass") == 0)
                {
                    if (!PReadWrite->FindString("EnteromorphaSpNBiomass values", XV, YV))
                    {
                        XV = X + VARIABLENAMECOLUMN + i;
                        YV = Y;
                    }
                    for (int j = 0; j < NumberOfBoxesWithEnteromorphaSp; j++)
                    {
                        ...
                        //if (j == NumberOfBoxesWithEnteromorphaSp - 1) Debugger(NPhyto[Index]);
                    }
                }
                else
                if (strcmp(MyVariable, "EnteromorphaSpPBiomass") == 0)
                {
                    if (!PReadWrite->FindString("EnteromorphaSpPBiomass values", XV, YV))
                    {
                        XV = X + VARIABLENAMECOLUMN + i;
                        YV = Y;
                    }
                    for (int j = 0; j < NumberOfBoxesWithEnteromorphaSp; j++)
                    {
                        ...
                        //if (j == NumberOfBoxesWithEnteromorphaSp - 1) Debugger(PPhyto[Index]);
                    }
                }
            //More variables here
            }
        ...
        {
#ifdef __BORLANDC__
            MessageBox(0, "Undefined object variables - EnteromorphaSp", "EcoDynamo alert", MB_OK);
#else  // __BORLANDC__
						cerr << "TEnteromorphaSp::TEnteromorphaSp - Undefined object variables" << endl;
#endif  // __BORLANDC__
        }
...
//	PReadWrite = (TReadWrite*)MyPEcoDynClass->GetParmsFileHandle();
    PReadWrite = (TReadWrite*)MyPEcoDynClass->OpenParametersFile("EnteromorphaSp");
    if (PReadWrite == NULL)
    {
#ifdef __BORLANDC__
		MessageBox(0, "EnteromorphaSp parameters file missing.", "EcoDynamo alert", MB_OK);
#else  // __BORLANDC__
		cerr << "TEnteromorphaSp::TEnteromorphaSp - parameters file missing." << endl;
#endif  // __BORLANDC__
	}
    else {
        double MyValue;
    ...
    //
    // List of parameters initialized in TRuppiaCirrhosa is not valid
    //
        ...
        {
#ifdef __BORLANDC__
            MessageBox(0, "Undefined object parameters - EnteromorphaSp", "EcoDynamo alert", MB_OK);
#else  // __BORLANDC__
						cerr << "TEnteromorphaSp::TEnteromorphaSp - Undefined object parameters" << endl;
#endif  // __BORLANDC__
        }
        CloseDataFile((void*)PReadWrite);
    }
...
TEnteromorphaSp::~TEnteromorphaSp()
{
        freeMemory();
}

void TEnteromorphaSp::freeMemory()
{
    try {
       if (NumberOfBoxes > 0)
       {
       ...
       if (NumberOfBoxesWithEnteromorphaSp > 0)
          delete [] EnteromorphaSpRecord;
    }
#ifdef __BORLANDC__
        ...
        //Application->ShowException(&exception);
				...
				cerr << "TEnteromorphaSp::freeMemory - " << exc.what() << endl;
		}
#endif  // __BORLANDC__
}

double TEnteromorphaSp::GetParameterValue(char* MyParameter)
{
  double value;
...
bool TEnteromorphaSp::SetParameterValue(char* MyParameter, double value)
{
  bool rc = true;
  int j;
...
void TEnteromorphaSp::Inquiry(char* srcName, double &Value,int BoxNumber,char* ParameterName, int AnObjectCode)
{
	char MyParameter[65]; // 64 characters (max) for parameter name
	int MyBoxNumber = BoxNumber, JulianDay;
	strcpy(MyParameter, ParameterName);

   	if (strcmp(MyParameter, "EnteromorphaSpBiomass") == 0)
    {
            //Debugger(PhytoBiomass[MyBoxNumber]);
        Value = PhytoBiomass[MyBoxNumber];
    }
    else if (strcmp(MyParameter, "EnteromorphaSpCBiomass") == 0)
        Value = CPhyto[MyBoxNumber];
    else if (strcmp(MyParameter, "EnteromorphaSpNBiomass") == 0)
        Value = NPhyto[MyBoxNumber];
    else if (strcmp(MyParameter, "EnteromorphaSpPBiomass") == 0)
        Value = PPhyto[MyBoxNumber];
    else if (strcmp(MyParameter, "EnteromorphaSp GPP per unit mass") == 0)
    {
	...
	else if (strcmp(MyParameter, "EnteromorphaSp NPP per unit mass") == 0)
    {
        if (PhytoBiomass[MyBoxNumber] == 0)
            Value = 0;
        else
    ...
    else if (strcmp(MyParameter, "EnteromorphaSpNCellQuota") == 0)
        Value = NCellQuota[MyBoxNumber];
    else if (strcmp(MyParameter, "EnteromorphaSpPCellQuota") == 0)
    ...
    else
    {
        ...
        if (j == NumberOfVariables)
        {
            ...
            MessageBox(0,
                  ...
                  MB_OK | MB_ICONHAND);
#else  // __BORLANDC__
						cerr << "TEnteromorphaSp::Inquiry 1 - " << MyParameter << " does not exist in " << GetEcoDynClassName() << endl;
#endif  // __BORLANDC__
        }
    }
...
bool TEnteromorphaSp::SetVariableValue(char* srcName, double Value,
									      int BoxNumber,
									     char* VariableName)
{
  bool rc = true;
    ...
    LogMessage("SetVariableValue", srcName, VariableName, Value, BoxNumber);
   	...
   	if (strcmp(VariableName, "EnteromorphaSpBiomass") == 0)
        PhytoBiomass[BoxNumber] = Value;
    else if (strcmp(VariableName, "EnteromorphaSpCBiomass") == 0)
        CPhyto[BoxNumber] = Value;
    else if (strcmp(VariableName, "EnteromorphaSpNBiomass") == 0)
        NPhyto[BoxNumber] = Value;
    else if (strcmp(VariableName, "EnteromorphaSpPBiomass") == 0)
        PPhyto[BoxNumber] = Value;
    else if (strcmp(VariableName, "EnteromorphaSp GPP per unit mass") == 0)
        GPP[BoxNumber] = Value / HOURSTOSECONDS;
	else if (strcmp(VariableName, "EnteromorphaSp NPP per unit mass") == 0)
        NPP[BoxNumber] = Value / HOURSTOSECONDS;
    else if (strcmp(VariableName, "EnteromorphaSpNCellQuota") == 0)
        NCellQuota[BoxNumber] = Value;
    else if (strcmp(VariableName, "EnteromorphaSpPCellQuota") == 0)
...
// AP, 2007.07.13
bool TEnteromorphaSp::SaveVariables()
{
	TReadWrite* PReadWrite = (TReadWrite*)SaveVariablesFile("EnteromorphaSp");
    if (PReadWrite == NULL)
    {
        return false;
    }
     ...
     * header file
     */
    PReadWrite->WriteCell("EnteromorphaSp");
    ...
    /*
     * EnteromorphaSp variables initial values
     */
    PReadWrite->WriteCell("EnteromorphaSpBiomass values");
    PReadWrite->WriteSeparator();
    PReadWrite->WriteCell("EnteromorphaSpCBiomass values");
    PReadWrite->WriteSeparator();
    PReadWrite->WriteCell("EnteromorphaSpNBiomass values");
    PReadWrite->WriteSeparator();
    PReadWrite->WriteCell("EnteromorphaSpPBiomass values");
    PReadWrite->WriteSeparator(true);
    int Index;
    for (int j = 0; j < NumberOfBoxesWithEnteromorphaSp; j++)
    {
...
bool TEnteromorphaSp::SaveParameters()
{
	TReadWrite* PReadWrite = (TReadWrite*)SaveParametersFile("EnteromorphaSp");
    if (PReadWrite == NULL)
    {
        return false;
    }
...
void TEnteromorphaSp::Go()
{
    ...
    SubDomain *pSubDomain = MyPEcoDynClass->GetSubDomain();
        //Debugger(NumberOfBoxesWithEnteromorphaSp);
        for (int m = 0; m < NumberOfBoxesWithEnteromorphaSp; m++)
        {
                i = EnteromorphaSpRecord[m].LineCoordinate;
                j = EnteromorphaSpRecord[m].ColumnCoordinate;
                if (
...
void TEnteromorphaSp::Integrate()
{
    int i, j, index;
...
void TEnteromorphaSp::Production(int ABoxNumber)
{
...
void TEnteromorphaSp::Respiration(int ABoxNumber)
{
   double Resp;
...
void TEnteromorphaSp::NitrogenUptake(int ABoxNumber)
{
...
void TEnteromorphaSp::PhosphorusUptake(int ABoxNumber)
{
...
double TEnteromorphaSp::NutrientLimitation(int ABoxNumber)
{
   double ALimitation;
   ALimitation =  MIN(
...
void TEnteromorphaSp::Mortality(int ABoxNumber)      //Solidoro et al. (1997) - Ecol. Modell. 94: 191-206
{
